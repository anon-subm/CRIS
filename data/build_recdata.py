...
def replace_id2idx(trn, vld, tst):
    ...
    def id2idx(uir, udict, idict):
        newuir = []
        for i in range(len(uir)):
            user, item, rating, _ = uir[i]
            uir[i][0] = udict[user]                
            uir[i][1] = idict[item]
    ...
    trn = id2idx(trn, user_dict, item_dict) # Exclude time stamps
...
def find_negatives(dataset):
    ...
    for i in range(len(vld)):
        ...
        negative_items = random.sample(allitems - useritems, NUMNEG)
        
        vld[i] = vld[i][:-1] + negative_items 
